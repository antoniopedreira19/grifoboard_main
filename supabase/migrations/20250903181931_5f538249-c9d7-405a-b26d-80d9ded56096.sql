...
CREATE INDEX IF NOT EXISTS idx_tarefas_obra_usuario ON public.tarefas(obra_id) 
  WHERE obra_id IN (SELECT id FROM public.obras WHERE usuario_id IS NOT NULL);
CREATE INDEX IF NOT EXISTS idx_atividades_obra_usuario ON public.atividades_checklist(obra_id)
  WHERE obra_id IN (SELECT id FROM public.obras WHERE usuario_id IS NOT NULL);